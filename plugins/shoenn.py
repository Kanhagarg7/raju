...
@bot.on(events.NewMessage(outgoing=True, pattern='.sahoenn'))
...
@bot.on(events.NewMessage(outgoing=True, pattern='.stoph'))
async def stop(event):
    global hoenn
    hoenn = False

@bot.on(events.NewMessage(from_users=chat, func=lambda e: "You have run out of Safari Balls and are now exiting the Hoenn Safari Zone" in e.text or "The Safari Game has finished and you were kicked out" in e.text))
async def stoppp(event):
    global hoenn
    hoenn = False

@bot.on(events.MessageEdited(from_users=chat, func=lambda e: "You have run out of Safari Balls and are now exiting the Hoenn Safari Zone" in e.text or "The Safari Game has finished and you were kicked out" in e.text))
async def stoppp(event):
    global hoenn
    hoenn = False