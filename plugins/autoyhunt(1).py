...
from . import LOGS, udB, kanha_bot, kanha_cmd
...
async def start_hunt(pokes: list):
    async with kanha_bot.conversation(HEXA_BOT, timeout=10) as conv:
...
async def main(pokes):
    ...
    async with kanha_bot.conversation(HEXA_BOT, timeout=90) as conv:
