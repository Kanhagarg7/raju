...
from . import *
#edit the list
list = ["Necrozma", "Marshadow", "Magearna", "Zeraora", "✨"]


@kanha_bot.on(events.NewMessage(outgoing=True,pattern='.salola'))
async def autohexa(e):
    i = True
    await e.edit("alola safari is on the way")
    if i == True:
...
@kanha_bot.on(events.NewMessage(from_users=chat,func=lambda e: "You have run out of Safari Balls and are now exiting the Alola Safari Zone" in e.text or "The Safari Game has finished and you were kicked out" in e.text))
async def stoppp(event):
   global alola
   alola = False

@kanha_bot.on(events.MessageEdited(from_users=chat,func=lambda e: "You have run out of Safari Balls and are now exiting the Alola Safari Zone" in e.text or "The Safari Game has finished and you were kicked out" in e.text))
async def stoppp(event):
   global alola
   alola = False