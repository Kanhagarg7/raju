...
from . import *
#edit the list
list = ["Mewtwo", "Ho-oh", "Lugia", "✨"]


@kanha_bot.on(events.NewMessage(outgoing=True,pattern='.sajohto'))
async def autohexa(e):
    i = True
    await e.edit("johto safari is on the way")
    if i == True:
...
@kanha_bot.on(events.NewMessage(from_users=chat,func=lambda e: "You have run out of Safari Balls and are now exiting the Johto Safari Zone" in e.text or "The Safari Game has finished and you were kicked out" in e.text))
async def stoppp(event):
   global johto
   johto = False

@kanha_bot.on(events.MessageEdited(from_users=chat,func=lambda e: "You have run out of Safari Balls and are now exiting the Johto Safari Zone" in e.text or "The Safari Game has finished and you were kicked out" in e.text))
async def stoppp(event):
   global johto
   johto = False